classdef Chain < handle
    ...
    methods
        ...
        % function localPassivityAnalysis(obj)
        % 
        %     debugMode = 0;
        %     i = obj.chainId;
        %     n = obj.numOfInventories
        ...
        %     con1 = tag(P >= epsilon*I, tagName);
        %     constraintMats{end+1} = P;
        % 
        %     % To check passivity
        %     W = [P, P*A, P*B; 
        %         A'*P, P+C'*X_22*C, C'*X_21;
        %         B'*P, X_12*C, X_11];
        ...
        %     constraintTags{end+1} = tagName;
        %     con4 = tag(rho >= epsilon, tagName);
        %     constraintMats{end+1} = rho;
        % 
        %     constraints = [con1, con2, con3, con4];
        %     costFunction = - 1*nu - 1000*rho + 0*trace(P);  
        % 
        %     solverOptions = sdpsettings('solver', 'mosek', 'verbose', 0, 'debug', 0);
        % 
        %     sol = optimize(constraints, costFunction, solverOptions);
        % 
        %     statusLocalAnalysis = sol.problem == 0
        %     debugMode = ~statusLocalAnalysis;
        % 
        %     PVal = value(P);
        %     nuVal = value(nu)
        %     rhoVal = value(rho)
        ...
        function [statusLocal, nuVal, rhoVal, LNormVal, gammaTildeVal, deltaVal] = localControlDesign(obj,pVal,deltaCostCoef)
            ...
            % costFunction = - 1*nu + 1*rhoTilde + 1*gammaTilde - 1*delta + 0*trace(P) + 1*norm(K);  
